...
namespace PrismDemo.B.Services
{
    /// <summary>
    /// B剂数据服务实现类
    /// 负责从ConnectData字典中提取B剂所需数据形成新字典pd并更新数据库剂BData
    /// </summary>
    public class BDataService : IDisposable
    {
        ...
        /// 字段配置列表（从数据库加载）
        /// </summary>
        public List<BFieldConfig> _fieldConfigs = new();

        /// <summary>
        /// 上一次B剂数据
        /// </summary>
        public BData? LastBData { get; set; }

        /// <summary>
        /// 保存本轮B剂数据供下轮使用（深拷贝，与当前数据完全独立）
        /// </summary>
        public void SaveLastBData(BData? bData)
        {
            LastBData = CloneBData(bData);
        }

        private static BData? CloneBData(BData? source)
        {
        ...
        /// B剂数据更新后触发的事件
        /// </summary>
        public event Action BDataUpdated;
        ...
        public BDataService()
        {
            ...
            {
                Debug.WriteLine($"[B.BDataService] 构造时加载字段配置失败（模块仍可加载，等待后续重试）: {ex.Message}");
            }
        }
        ...
        private void Load_fieldConfigsFromDb()
        {
            try
            {
                ...
                var sql = "SELECT name, connectName, IsStore, IsFilter, FilterLength FROM BConfig";
                ...
                while (reader.Read())
                {
                    _fieldConfigs.Add(new BFieldConfig
                    {
                ...
                Debug.WriteLine($"[B.BDataService] 从数据库加载了 {_fieldConfigs.Count} 个字段配置");
            }
            catch (Exception ex)
            {
                Debug.WriteLine($"[B.BDataService] 加载字段配置失败: {ex.Message}");
                throw;
            }
        }
        ...
        /// <summary>
        /// 更新B剂数据
        /// 从ConnectData字典中提取数据并更新
        /// <param name="collectedData">连接配置数据字典</param>
        /// <param name="pd">B剂数据字典</param>
        /// <returns>更新后的pd</returns>
        public BData UpdateBData(Dictionary<string, ConnectData> collectedData, BData? pd)
        {
            if (collectedData == null || collectedData.Count == 0)
            {
                Debug.WriteLine("[BDataService] collectedData字典为空");
                return pd ?? new BData();
            }
            ...
            try
            {
                ...
                foreach (var config in _fieldConfigs)
                {
                        ...
                        collectedData.TryGetValue(config.ConnectName, out var connectData))
                    {
                        ...
                        {
                            //采用克隆而不是直接引用，避免后续数据变动影响已保存的B剂数据
                            pd[config.Name] = DatabaseService.CloneConnectData(connectData);
                        }
                        else
                        {
                            if (LastBData != null && LastBData.ContainsKey(config.Name))
                                pd[config.Name] = DatabaseService.CloneConnectData(LastBData[config.Name]);
                            else
                                pd[config.Name] = DatabaseService.CloneConnectData(connectData);
                        }
                    }
            ...
            {
                Debug.WriteLine($"[B.BDataService] 更新数据异常: {ex.Message}");
            }
        ...
        public void StopAutoUpdate()
        {
            _timer?.Dispose();
            _timer = null;
            Debug.WriteLine("[BDataService] 自动更新已停止");
        }
        ...
        public void EnsureTableStructure()
        {
            try
            {
                ...
                Debug.WriteLine("[B.EnsureTableStructure] 表结构已确保");
            }
            catch (Exception ex)
            {
                Debug.WriteLine($"[B.EnsureTableStructure] 失败: {ex.Message}");
                throw;
            }
        }
        ...
        /// </summary>
        public void SaveBData(BData aData)
        {
            ...
            try
            {
                ...
                if (!storeFields.Any())
                {
                    Debug.WriteLine("[SaveBData] 没有需要保存的字段");
                    return;
                }
                ...
                Trace.WriteLine($"[SaveBData] 数据已保存到数据库");
            }
            catch (Exception ex)
            {
                Trace.WriteLine($"[SaveBData] 保存失败: {ex.Message}");
        ...
        public List<HistoryPoint> GetHistoricalData(string[] fields, int hours = 24)
        {
            ...
            try
            {
                ...
                var sql = $"SELECT Datetime, {columns} FROM BData WHERE Datetime >= DATEADD(HOUR, -@hours, GETDATE()) ORDER BY Datetime";
        ...
        /// <summary>
        /// 按日期范围查询历史数据（历史曲线页使用）
        /// SQL: SELECT Datetime, {fields} FROM BData WHERE Datetime BETWEEN @start AND @end
        /// </summary>
        ...
        public List<HistoryPoint> GetHistoricalDataByRange(string[] fields, DateTime start, DateTime end)
        {
            ...
            try
            {
                ...
                var columns = string.Join(",", fields.Select(f => $"[{f}]"));
                var sql = $"SELECT Datetime, {columns} FROM BData WHERE Datetime >= @start AND Datetime <= @end ORDER BY Datetime";
            ...
            {
                Debug.WriteLine($"[B.GetHistoricalDataByRange] 查询失败: {ex.Message}");
            }
            return result;
        }

        public HistoryPoint? GetLatestData(string[] fields)
        {
            try
            {
                using var conn = new SqlConnection(Config.DatabaseConnectionString);
                ...
                var columns = string.Join(",", fields.Select(f => $"[{f}]"));
                var sql = $"SELECT TOP 1 Datetime, {columns} FROM BData ORDER BY Datetime DESC";
            ...
            {
                Debug.WriteLine($"[B.GetLatestData] 查询失败: {ex.Message}");
                return null;
            }
        }
        ...
        public ChartQueryResult GetHistoryChartData(string[] fields, DateTime start, DateTime end)
        {
            ...
            {
                Debug.WriteLine($"[B.GetHistoryChartData] 查询失败: {ex.Message}");
        ...
        private static Dictionary<string, string> LoadConnectTypeMap(
            SqlConnection conn, IEnumerable<BFieldConfig> storeFields)
        {
        ...
        /// <summary>
        /// 配合 SaveBData 方法使用，检查表中已有的列，避免重复添加
        /// </summary>
        ...
        /// <summary>
        /// 配合 SaveBData 方法使用，根据 ConnectData 的 Type 将 Value 转换为适合数据库存储的值
        /// </summary>
